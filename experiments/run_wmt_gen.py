"""WMT16 DE-EN generation with two decoding strategies.

Strategy 1: Sampling (temperature=0.7)
Strategy 2: Beam search (num_beams=8, no_repeat_ngram_size=16)

Usage:
  CUDA_VISIBLE_DEVICES=X python experiments/run_wmt_gen.py sample --max-samples 100
  CUDA_VISIBLE_DEVICES=X python experiments/run_wmt_gen.py beam   --max-samples 100
"""
...
from evaluation.dataset import WMT16DE_ENDataset
...
print(f"[PID={PID}] WMT16 DE-EN | {DECODE_NAME} | max_samples={MAX_SAMPLES}")
...
@torch.no_grad()
def compute_ppl(texts, batch_size=8):
    """Compute perplexity on generated texts using the model."""
...
gen_kwargs = {"max_new_tokens": MAX_NEW_TOKENS, **DECODE_KWARGS}
tcfg = TransformersConfig(model=model, tokenizer=tokenizer, vocab_size=len(tokenizer),
    device="cuda", **gen_kwargs)

ds = WMT16DE_ENDataset("dataset/wmt16_de_en/validation.jsonl", max_samples=MAX_SAMPLES)
N = ds.prompt_nums
print(f"WMT16 DE-EN: {N} samples | decoding: {DECODE_NAME}")

# ============================================================
# Step 1: Shared unwatermarked baseline
# ============================================================
...
print(f"  Saved: {LOG_DIR}/baseline.jsonl  |  PPL(non)={non_ppl:.2f}")

# ============================================================
# Step 2: Watermark methods
# ============================================================
...
for alg_name, tag, cfg in RUNS:
    ...
    if isinstance(cfg, dict):
        tmp = f"results/tmp_run/{alg_name}_wmt_{DECODE_NAME}.json"
    ...
    with open(jl_path, "w", encoding="utf-8") as f:
        for i in range(N):
            f.write(json.dumps({"idx": i, "prompt": ds.get_prompt(i),
                                "watermarked_text": wm_texts[i]}, ensure_ascii=False) + "\n")
    print(f"  Saved: {jl_path}")

    # Built-in detection
    wm_scores, non_scores = [], []
    for t in tqdm(wm_texts, desc=f"{tag} detect wm"):
        s = safe_detect(wm, t)
        if s is not None: wm_scores.append(s)
    for t in tqdm(non_texts, desc=f"{tag} detect non"):
        s = safe_detect(wm, t)
    ...
    print(f"  AUROC={m['AUROC']} TPR1={m['TPR@1%']} F1@1={m['F1@1%']} TPR5={m['TPR@5%']} F1@5={m['F1@5%']} BF1={m['Best-F1']} D={D} PPL={ppl} pairs={m['pairs']}")
    all_results.append({"algorithm": tag, "PPL": ppl, **m, "jsonl": jl_path})
...
summary = {"dataset": "WMT16 DE-EN", "model": MODEL, "decoding": DECODE_NAME,
           "max_samples": N, "results": all_results}
